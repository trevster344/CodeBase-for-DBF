...
#include "d4all.h"
   ...
   void *S4FUNCTION v4Cstring(char *) ;
...
#ifdef S4WINTEL
   /* Allocates a native copy of a VB string for the *CB structures used by the VB interface
      (d4create/i4create/d4fieldsAdd/...). Returns a pointer - the historical 'long' return
      type was wrong on 64-bit (long is 32-bit there), so this was previously stubbed to
      return 0, which broke those wrappers. Callers free the result with v4Cstringfree(). */
   void *S4FUNCTION v4Cstring(char *s)
   {
      char *d = 0 ;
